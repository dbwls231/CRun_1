...
AnimationRenderData* AnimationRenderData::PushTextures(Textures textures) 
{
	animations.push_back(textures);
	SetVisibleArea();

	return this;
}

AnimationRenderData* AnimationRenderData::PushTextures(const std::string& path) 
{
	Textures textures;

	for (auto& p : std::experimental::filesystem::recursive_directory_iterator(path)) {
		textures.PushTexture(path + "/" + p.path().filename().string());
	}
